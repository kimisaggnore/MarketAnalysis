...
def save_html_page(ticker, setting):
    ...
    with open(os.path.join(sys.path[0], f"html_files/company_balance_sheets_html/saved_{ticker}_balance_sheet{append_2}.txt"), "w") as file:
...
def write_balance_sheet_to_csv(ticker, setting):
    ...
    with open(os.path.join(sys.path[0], f'{append_2}_statements/company_balance_sheets{append_2}/{ticker}_balance_sheet{append_2}.csv'), mode = "w", newline = '') as company_fundamental_valuation:
        ...
        company_profile = open(os.path.join(sys.path[0], f"html_files/company_balance_sheets_html/saved_{ticker}_balance_sheet{append_2}.txt"), "r")
...
def rename_all():
    path = os.path.join(sys.path[0],"html_files/company_balance_sheets_html")
